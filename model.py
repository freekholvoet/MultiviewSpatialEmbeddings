...
class LayerNorm(nn.LayerNorm):
    ...
    def forward(self, x: torch.Tensor):
        orig_type = x.dtype
        ret = super().forward(x.type(dtype=torch.get_default_dtype()))
...
def create_hex_mask(k):
    ...
    mask = torch.ones((k, k), dtype=torch.get_default_dtype())
...
class SatCLIP(nn.Module):
    def __init__(self, 
        ...
        self.nnet = get_neural_network(
            ...
            num_classes=Combined_dim,
            dim_hidden=loc_capacity,
            num_layers=loc_layers
        ).to(torch.get_default_dtype())

        self.location = LocationEncoder(self.posenc, self.nnet).to(torch.get_default_dtype())
    ...
    def encode_location(self, coords):
        return self.location(coords.to(torch.get_default_dtype()))

    def forward(self, gs, osm, s2, fm, coords):
        features = []

        if self.GSview is not None:
            features.append(self.encode_GS(gs.to(torch.get_default_dtype())))
        if self.OSMview is not None:
            features.append(self.encode_OSM(osm.to(torch.get_default_dtype())))
        if self.S2view is not None:
            features.append(self.encode_s2(s2.to(torch.get_default_dtype())))
        if self.FMview is not None:
            features.append(self.encode_fm(fm.to(torch.get_default_dtype())))

        combined_features = self.encode_combined(torch.cat(features, dim=1).to(torch.get_default_dtype()))
        location_features = self.encode_location(coords.to(torch.get_default_dtype()))
