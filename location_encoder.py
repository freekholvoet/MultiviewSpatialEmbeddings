...
def get_positional_encoding(name, legendre_polys=10, harmonics_calculation='analytic', min_radius=1, max_radius=360, frequency_num=10):
    if name == "direct":
        print('Using direct as positional encoding')
        return PE.Direct()
    elif name == "w3034":
        print('Using W3034 as positional encoding')
        return PE.W3034()
    elif name == "cartesian3d":
        print('Using Cartesian3D as positional encoding')
        return PE.Cartesian3D()
    elif name == "sphericalharmonics":
        if harmonics_calculation == 'discretized':
            print(f'Using DiscretizedSphericalHarmonics as positional encoding with {legendre_polys} legendre polynomials')
            return PE.DiscretizedSphericalHarmonics(legendre_polys=legendre_polys)
        else:
            print(f'Using SphericalHarmonics as positional encoding with {legendre_polys} legendre polynomials')
            return PE.SphericalHarmonics(legendre_polys=legendre_polys,
                                         harmonics_calculation=harmonics_calculation)
    elif name == "theory":
        print('Using Theory as positional encoding')
        return PE.Theory(min_radius=min_radius,
                         max_radius=max_radius,
                         frequency_num=frequency_num)
    elif name == "wrap":
        print('Using Wrap as positional encoding')
        return PE.Wrap()
    elif name in ["grid", "spherec", "spherecplus", "spherem", "spheremplus"]:
        print(f'Using GridAndSphere as positional encoding')
...
def get_neural_network(name, input_dim, num_classes=256, dim_hidden=256, num_layers=2):
    if name == "linear":
        print('Using Linear as to process positional encoding')
        return nn.Linear(input_dim, num_classes)
    elif name ==  "mlp":
        print('Using MLP as to process positional encoding')
        return MLP(
                input_dim=input_dim,
        ...
        print('Using Siren as to process positional encoding')
        return SirenNet(
                dim_in=input_dim,
                dim_hidden=dim_hidden,
                num_layers=num_layers,
        ...
        print('Using FCNet as to process positional encoding')
        return FCNet(
