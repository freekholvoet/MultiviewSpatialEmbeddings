...
def get_s2_train_transform(resize_crop_size = 256):
    ...
    def augmentation_imageVec(image):
        ...
        return torch.tensor(random_column, dtype=torch.get_default_dtype())
    ...
    def transform(sample):
        ...
        if len(image.shape) == 2: #Vectorised image
            ...
            image = torch.tensor(image, dtype=torch.get_default_dtype())
            image = augmentation_image(image)

        else: # RGB image
            image = image / 255
            image = torch.tensor(image, dtype=torch.get_default_dtype()).permute(2,0,1)
            image = augmentation_image(image)
...
def mv_noaugment_transform(sample, resize_crop_size=256):
    augmentation = T.Compose([
        T.RandomCrop(resize_crop_size)
    ])
    ...
    if sample['gs'] is not None:
        gs = sample['gs']
        gs = gs / 255
        gs = torch.tensor(gs, dtype=torch.get_default_dtype()).permute(2, 0, 1)
    else:
        gs = torch.empty((0), dtype=torch.get_default_dtype())
    ...
    if sample['osm'] is not None:
        ...
        osm = torch.empty((0), dtype=torch.get_default_dtype())
    ...
    if sample['s2'] is not None:
        ...
        s2 = torch.tensor(s2, dtype=torch.get_default_dtype())
        s2 = augmentation(s2)
    else:
        s2 = torch.empty((0), dtype=torch.get_default_dtype())

    if sample['fm'] is not None:
        fm = sample['fm']
    else:
        fm = torch.empty((0), dtype=torch.get_default_dtype())
...
def mv_noaugment(resize_crop_size=256):
    def transform(sample):
        return mv_noaugment_transform(sample, resize_crop_size)
    return transform

def mv_noaugment_OLD(resize_crop_size = 256):
...
class ExtendedTensor(torch.Tensor):
    def normalized(self, divisors):
        ...
        if isinstance(divisors, list):
            divisors = torch.tensor(divisors, dtype=torch.get_default_dtype())
...
def create_3d_tensor_from_osm_data(osm_data, columns_of_interest=['health', 'shops', 'tourism', 'food_and_drink', 'education', 'public_services']):
    ...
    tensor_3d = torch.tensor(tensor_3d, dtype=torch.get_default_dtype())
